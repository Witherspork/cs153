...
(*
  ...
  In this tutorial-style project, if you encounter an OCaml concept or
  construct that you are not familiar with you, you may find it
  helpful to consult an OCaml reference book, such as the books and
  tutorials linked to from the course's resources page. Two such books
  that may be useful (and are available online) are "Introduction to
  Objective Caml", by Jason Hickey and "Real World OCaml" 2nd edition
  by Yaron Minsky, Anil Madhavapeddy, and Jason Hickey. We will
  abbreivate these as IOC and RWO respectively in the notes below.
 
  
  Please also take some time to skim the other available resources on the course 
  webpage -- there are links to the standard libraries, and on-line tutorials. 
  ...
  Also, please feel ask a course staff member for help if you get
  stuck. Come to office hours, or post on Piazza. Learning OCaml is
  essential for your successful completion of later projects, but it
  is not supposed to be too onerous in itself!
*)
...
(*
  ...
  For more details, see, e.g., IOC Chapter 2, pay particular attention to 2.4 about
  typechecking.
*)
...
(*
  ...
  See also, e.g., RWO "Variables and Functions" or IOC Chapter 3.
*)
...
(* 
  ...
  See, e.g., IOC Chapter 3.1 or RWO "Variables and Functions"
...
(*
  ...
  When you run ocaml from the command line you should see:
...
(* 
  ...
  Follow the instructions on the homework web page.
...
(* 
  NOTE: See, e.g., IOC 5.2 or RWO "A Guided Tour - Tuples, Lists,
...
(* 
  ...
  Pattern matching is a fundamental concept in OCaml: most non-trivial
  OCaml types are usually "destructed" or "taken apart" by pattern
  matching using the 'match-with' notation.  See, e.g., IOC Chapter 4.
...
(*
  ...
  type of an expression at the cursor: ^C ^T on Emacs
*)
...
(*
  OCaml has a build-in datatype of generic lists: See, e.g., IOC 5.3
...
(*
  ...
  Read IOC 5.4 about "tail recursion" or RWO "Lists and Patterns -
  Tail Recursion" and implement a tail recursive version of rev.  Note
  that you will need a helper function that takes an extra parameter
  -- it should be defined using a local let definition.  The rev_t
  function itself should not be recursive. Tail recursion is important
